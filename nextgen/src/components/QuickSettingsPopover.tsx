...
export function QuickSettingsPopover({ type }: QuickSettingsPopoverProps) {
  ...
  useEffect(() => {
    ...
    controls.forEach((control) => {
      ...
      const rowLabel = control.closest('.qs-row')?.querySelector<HTMLElement>('.qs-row-label')?.textContent?.trim();
      const sectionLabel = control.closest('.qs-section')?.querySelector<HTMLElement>('.qs-section-header')?.textContent?.trim();
      const placeholder = control.getAttribute('placeholder')?.trim();
      control.setAttribute('aria-label', rowLabel || sectionLabel || placeholder || getTitle());
    });
  }, [config, isOpen, type]);
  ...
  const overlayContent = isOpen ? (
    <div className="qs-overlay">
      <div
        className={`qs-modal qs-modal--${type}`}
        ref={modalRef}
        role="dialog"
        aria-modal="true"
        aria-label={getTitle()}
      >
        <div className="qs-header">
          ...
          <button type="button" className="qs-close" onClick={() => setIsOpen(false)} aria-label={t('common.close')}>
            <X size={16} />
          </button>
        </div>
        ...
        {error && (
          <div className="qs-error">
            {error}
            <button type="button" className="qs-error-close" onClick={() => setError(null)} aria-label={t('common.close')}>
              <X size={12} />
            </button>
          </div>
        )}
              ...
              <div className="qs-section">
                ...
                <div className="qs-hint">
                  {t(
                    ...
                    '重启 Codex 实例后，在输入框下方显示 XIASS Tools API 服务的账号数、周额度和 5h 额度。需保持 XIASS Tools 在后台运行；完全退出或网络不可用时，额度不会继续刷新。',
                  )}
                </div>
                <div className="qs-row" style={{ marginTop: 8 }}>
                  <div className="qs-row-label">
