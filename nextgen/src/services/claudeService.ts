...
function normalizeClaudeDesktopLoginStartResponse(
  raw: ClaudeDesktopLoginStartResponseRaw | null | undefined,
): ClaudeDesktopLoginStartResponse {
...
function normalizeClaudeOAuthStartResponse(
  raw: ClaudeOAuthStartResponseRaw | null | undefined,
): ClaudeOAuthStartResponse {
  if (!raw || typeof raw !== 'object') {
    throw new Error('原生授权服务未返回有效会话信息，请稍后重试');
  }
...
export async function claudeDesktopLoginStart(progressId?: string): Promise<ClaudeDesktopLoginStartResponse> {
  const raw = await invoke<ClaudeDesktopLoginStartResponseRaw | null>('claude_desktop_login_start', {
...
export async function claudeOauthLoginStart(): Promise<ClaudeOAuthStartResponse> {
  const raw = await invoke<ClaudeOAuthStartResponseRaw | null>('claude_oauth_login_start');
...
export async function claudeOauthLoginPrepare(): Promise<ClaudeOAuthStartResponse> {
  const raw = await invoke<ClaudeOAuthStartResponseRaw | null>('claude_oauth_login_prepare');
  return normalizeClaudeOAuthStartResponse(raw);
}
