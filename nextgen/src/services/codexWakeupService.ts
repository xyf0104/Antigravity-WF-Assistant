...
function requireWakeupResponse<T>(raw: T | null | undefined): T {
  if (raw == null) {
    throw new Error('原生唤醒服务未返回运行状态，请稍后重试');
  }
  return raw;
}
...
export async function getCodexWakeupCliStatus(): Promise<CodexCliStatus> {
  const raw = await invoke<RawCodexCliStatus | null>('codex_wakeup_get_cli_status');
  return fromRawCliStatus(requireWakeupResponse(raw));
}
...
export async function updateCodexWakeupRuntimeConfig(
  ...
  return fromRawCliStatus(
    requireWakeupResponse(await invoke<RawCodexCliStatus | null>('codex_wakeup_update_runtime_config', {
...
export async function getCodexWakeupOverview(): Promise<CodexWakeupOverview> {
  const raw = await invoke<RawCodexWakeupOverview | null>('codex_wakeup_get_overview');
  return fromRawOverview(requireWakeupResponse(raw));
}

export async function getCodexWakeupState(): Promise<CodexWakeupState> {
  const raw = await invoke<RawCodexWakeupState | null>('codex_wakeup_get_state');
  return fromRawState(requireWakeupResponse(raw));
}
...
export async function saveCodexWakeupState(
  ...
  return fromRawState(
    requireWakeupResponse(await invoke<RawCodexWakeupState | null>('codex_wakeup_save_state', {
      enabled,
...
export async function loadCodexWakeupHistory(): Promise<CodexWakeupHistoryItem[]> {
  const raw = await invoke<RawCodexWakeupHistoryItem[] | null>('codex_wakeup_load_history');
  return Array.isArray(raw) ? raw.map(fromRawHistoryItem) : [];
}
...
export async function testCodexWakeup(
  ...
  return fromRawBatchResult(
    requireWakeupResponse(await invoke<RawCodexWakeupBatchResult | null>('codex_wakeup_test', {
      accountIds,
      runId,
      prompt: prompt ?? null,
      model: model ?? null,
...
export async function runCodexWakeupTask(taskId: string, runId: string): Promise<CodexWakeupBatchResult> {
  return fromRawBatchResult(
    requireWakeupResponse(
      await invoke<RawCodexWakeupBatchResult | null>('codex_wakeup_run_task', { taskId, runId }),
    ),
  );
}
