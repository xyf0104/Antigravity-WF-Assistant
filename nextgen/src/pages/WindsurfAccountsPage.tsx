        ...
        <div key={groupKey ? `${groupKey}-${account.id}` : account.id} className={`ghcp-account-card ${isCurrent ? 'current' : ''} ${isSelected ? 'selected' : ''}`}>
          ...
          <div className="card-footer">
            ...
            <div className="card-actions">
              <button className="card-action-btn success" onClick={() => handleInjectToVSCode?.(account.id)} disabled={!!injecting} title={t('windsurf.injectToVSCode', '切换到 Windsurf')} aria-label={t('windsurf.injectToVSCode', '切换到 Windsurf')}>
                {injecting === account.id ? <RefreshCw size={14} className="loading-spinner" /> : <Play size={14} />}
              </button>
              <button className="card-action-btn" onClick={() => openTagModal(account.id)} title={t('accounts.editTags', '编辑标签')} aria-label={t('accounts.editTags', '编辑标签')}><Tag size={14} /></button>
              <button className="card-action-btn" onClick={() => handleRefresh(account.id)} disabled={refreshing === account.id} title={t('common.shared.refreshQuota', '刷新配额')} aria-label={t('common.shared.refreshQuota', '刷新配额')}>
              ...
              <button className="card-action-btn danger" onClick={() => handleDelete(account.id)} title={t('common.delete', '删除')} aria-label={t('common.delete', '删除')}><Trash2 size={14} /></button>
            </div>
          </div>
        </div>
      );
        ...
        <tr key={groupKey ? `${groupKey}-${account.id}` : account.id} className={isCurrent ? 'current' : ''}>
          ...
          <td className="sticky-action-cell table-action-cell">
            <div className="action-buttons">
              <button className="action-btn success" onClick={() => handleInjectToVSCode?.(account.id)} disabled={!!injecting} title={t('windsurf.injectToVSCode', '切换到 Windsurf')} aria-label={t('windsurf.injectToVSCode', '切换到 Windsurf')}>
                {injecting === account.id ? <RefreshCw size={14} className="loading-spinner" /> : <Play size={14} />}
              </button>
              <button className="action-btn" onClick={() => openTagModal(account.id)} title={t('accounts.editTags', '编辑标签')} aria-label={t('accounts.editTags', '编辑标签')}><Tag size={14} /></button>
              <button className="action-btn" onClick={() => handleRefresh(account.id)} disabled={refreshing === account.id} title={t('common.shared.refreshQuota', '刷新配额')} aria-label={t('common.shared.refreshQuota', '刷新配额')}>
                <RotateCw size={14} className={refreshing === account.id ? 'loading-spinner' : ''} />
              </button>
              <button
              ...
              <button className="action-btn danger" onClick={() => handleDelete(account.id)} title={t('common.delete', '删除')} aria-label={t('common.delete', '删除')}><Trash2 size={14} /></button>
            </div>
          </td>
        </tr>
      );
    ...
    <div className="ghcp-accounts-page windsurf-accounts-page">
      ...
      {message && (
        <div className={`message-bar ${message.tone === 'error' ? 'error' : 'success'}`}>
          {message.text}
          <button type="button" onClick={() => setMessage(null)} aria-label={t('common.close', '关闭')}><X size={14} /></button>
        </div>
      )}

      <div className="toolbar">
        <div className="toolbar-left">
          <div className="search-box"><Search size={16} className="search-icon" /><input type="text" aria-label={t('common.shared.search', '搜索账号...')} placeholder={t('common.shared.search', '搜索账号...')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
          <div className="view-switcher">
            <button className={`view-btn ${viewMode === 'list' ? 'active' : ''}`} onClick={() => setViewMode('list')} title={t('common.shared.view.list', '列表视图')} aria-label={t('common.shared.view.list', '列表视图')} aria-pressed={viewMode === 'list'}><List size={16} /></button>
            <button className={`view-btn ${viewMode === 'grid' ? 'active' : ''}`} onClick={() => setViewMode('grid')} title={t('common.shared.view.grid', '卡片视图')} aria-label={t('common.shared.view.grid', '卡片视图')} aria-pressed={viewMode === 'grid'}><LayoutGrid size={16} /></button>
          </div>
        ...
        <div className="toolbar-right">
          ...
          <button className="btn btn-secondary export-btn icon-only" onClick={() => void handleExport(filteredIds)} disabled={exporting || filteredIds.length === 0}
            ...
            aria-label={exportSelectionCount > 0 ? `${t('common.shared.export.title', '导出')} (${exportSelectionCount})` : t('common.shared.export.title', '导出')}><Upload size={14} /></button>
      ...
      {showAddModal && (
        <div className="modal-overlay"><div
          className="modal-content ghcp-add-modal windsurf-add-modal"
          role="dialog"
          aria-modal="true"
          aria-labelledby="windsurf-add-account-title"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="modal-header"><h2 id="windsurf-add-account-title">{t('windsurf.addModal.title', '添加 Windsurf 账号')}</h2><button className="modal-close" onClick={handleCloseAddModal} aria-label={t('common.close', '关闭')}><X /></button></div>
      ...
      {deleteConfirm && (
        <div className="modal-overlay"><div className="modal" onClick={(e) => e.stopPropagation()}>
