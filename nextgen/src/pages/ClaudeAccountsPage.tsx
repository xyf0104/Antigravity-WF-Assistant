...
export function ClaudeAccountsPage({ subPlatform = 'desktop' }: ClaudeAccountsPageProps) {
  ...
  useEffect(() => {
    const handleWorkspaceNavigation = (event: Event) => {
      ...
      if (detail?.platform !== 'claude-code') return;
      if (!['desktop', 'cli', 'instances'].includes(detail.tab)) return;
      setActiveSection(detail.tab as ClaudePageSection);
    };
  ...
  const prepareOAuthLogin = useCallback(async (force = false): Promise<ClaudeOAuthStartResponse | null> => {
    if (oauthLogin && !force) return oauthLogin;
    ...
    <div className="ghcp-accounts-page codex-accounts-page claude-accounts-page">
            ...
            <div className="toolbar-left">
              ...
              <div className="view-switcher">
                <button
                  ...
                  aria-label={t('accounts.view.list', '列表视图')}
                >
                  <List size={16} />
                </button>
                <button
                  ...
                  aria-label={t('accounts.view.grid', '卡片视图')}
                >
                  <LayoutGrid size={16} />
                </button>
              </div>
                  ...
                  <div className="codex-overview-selection-actions">
                    {selectedExportableIds.length > 0 && (
                      <button
                        ...
                        aria-label={`${t('common.shared.export.title', '导出')} (${selectedExportableIds.length})`}
                      >
                        <Upload size={14} />
                      </button>
                    )}
                    <button
                      ...
                      aria-label={`${t('common.delete', '删除')} (${selectedDeletableIds.length})`}
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
      ...
      {showAddModal && createPortal(
        <div className="modal-overlay">
          <div
            className="modal ghcp-add-modal claude-add-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="claude-add-account-title"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="modal-header">
              <h2 id="claude-add-account-title">
                {t(
                  editingDesktopGatewayAccountId
              ...
              {addTab === 'oauth' && (
                <div className="add-section">
                  ...
                  <div className="oauth-url-section">
                    ...
                    {!oauthLogin && !oauthStarting && (
                      <button
                        type="button"
                        className="btn btn-secondary btn-full"
                        onClick={() => void prepareOAuthLogin(true)}
                      >
                        <RefreshCw size={14} />
                        {t('claude.oauth.retryPrepare', '重新生成授权信息')}
                      </button>
                    )}
                    <p className="section-desc">
                      {t(
