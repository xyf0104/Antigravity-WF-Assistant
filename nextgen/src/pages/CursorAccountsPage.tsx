          ...
          <div className="card-footer">
            ...
            <div className="card-actions">
              <button className="card-action-btn success" onClick={() => handleInjectToVSCode?.(account.id)} disabled={!!injecting || isBanned}
                ...
                aria-label={isBanned ? t('accounts.status.forbidden_msg') : t('cursor.injectToCursor', '切换到 Cursor')}>
                {injecting === account.id ? <RefreshCw size={14} className="loading-spinner" /> : <Play size={14} />}
              </button>
              <button className="card-action-btn" onClick={() => openTagModal(account.id)} title={t('accounts.editTags', '编辑标签')} aria-label={t('accounts.editTags', '编辑标签')}>
                <Tag size={14} />
              </button>
              ...
              <button className="card-action-btn danger" onClick={() => handleDelete(account.id)} title={t('common.delete', '删除')} aria-label={t('common.delete', '删除')}>
                <Trash2 size={14} />
              </button>
            </div>
          </div>
        ...
        <tr key={groupKey ? `${groupKey}-${account.id}` : account.id} className={`${isCurrent ? 'current' : ''} ${isBanned ? 'disabled' : ''}`}>
          ...
          <td className="sticky-action-cell table-action-cell">
            <div className="action-buttons">
              ...
              <button className="action-btn" onClick={() => openTagModal(account.id)} title={t('accounts.editTags', '编辑标签')} aria-label={t('accounts.editTags', '编辑标签')}>
                <Tag size={14} />
              </button>
              ...
              <button className="action-btn danger" onClick={() => handleDelete(account.id)} title={t('common.delete', '删除')} aria-label={t('common.delete', '删除')}>
                <Trash2 size={14} />
              </button>
            </div>
          </td>
      ...
      <div className="toolbar">
        <div className="toolbar-left">
          <div className="search-box">
            ...
            <input type="text" aria-label={t('common.shared.search', '搜索账号...')} placeholder={t('common.shared.search', '搜索账号...')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
      ...
      {showAddModal && (
        <div className="modal-overlay">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="cursor-add-account-title"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="modal-header">
              <h2 id="cursor-add-account-title">{t('cursor.addModal.title', '添加 Cursor 账号')}</h2>
