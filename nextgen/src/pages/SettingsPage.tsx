...
export function useSettingsPageController() {
  ...
  const menuBarQuotaPlatformOptions: Array<{ value: PlatformId; label: string }> = [
    ...
    { value: 'claude_manager', label: 'Claude Code' },
    { value: 'antigravity', label: 'Antigravity WF' },
  ...
  const [closeBehavior, setCloseBehavior] = useState<'ask' | 'minimize' | 'quit'>('minimize');
  ...
  const loadGeneralConfig = async () => {
    ...
    try {
      ...
      setCloseBehavior(config.close_behavior || 'minimize');
  ...
  const loadNetworkConfig = async () => {
    try {
      const config = await invoke<NetworkConfig | null>('get_network_config');
      if (!config) {
        return;
      }
  ...
  const loadDiagnosticsConfig = async () => {
    try {
      const config = await invoke<DiagnosticsConfig | null>('get_diagnostics_config');
      if (!config) {
        return;
      }
