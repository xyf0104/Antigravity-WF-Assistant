...
http_headers = { "x-openai-actor-authorization" = "xiass-tools", "x-agtools-disable-image-generation" = "chat", "x-cockpit-instance-id" = "default", "X-Custom" = "keep-me" }
...
"#;
        ...
        assert!(is_cockpit_managed_local_access_config(
            &input.replace("sk-user-custom", "xiass_codex_managed")
        ));
    }
...
http_headers = { "x-openai-actor-authorization" = "xiass-tools", "X-Custom" = "keep-current" }
"#;
...
"#,
    ...
    fn detects_only_matching_local_access_auth_key() {
        ...
        assert!(is_codex_local_access_auth_text(
            r#"{"auth_mode":"apikey","OPENAI_API_KEY":"xiass_codex_generated"}"#,
            "local-key"
        ));
        let generated = super::generate_local_api_key();
        assert!(generated.starts_with("xiass_codex_"));
        assert!(!is_codex_local_access_auth_text(
...
requires_openai_auth = false
experimental_bearer_token = "agt_codex_test"
http_headers = { "x-openai-actor-authorization" = "xiass-tools" }
supports_websockets = false
"#;
...
requires_openai_auth = true
experimental_bearer_token = "agt_codex_test"
http_headers = { "x-openai-actor-authorization" = "xiass-tools", "x-agtools-disable-image-generation" = "chat" }
supports_websockets = false
"#;
