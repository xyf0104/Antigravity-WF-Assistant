...
fn sanitize_sidecar_command_env(command: &mut TokioCommand) {
    #[cfg(target_os = "macos")]
    {
        // 避免把 XIASS Tools 的应用身份和 XPC 上下文传给 sidecar，导致局域网访问异常。
...
fn sync_sidecar_auth_file_for_account_with_task_source(
    ...
    logger::log_codex_api_info(&format!(
        "[CodexLocalAccess][sidecar] 已写入 XIASS Tools Token Authority 凭证: account_id={}",
        account.id
    ));
    Ok(())
}
...
/// True when `raw` is the XIASS API Service client URL (gateway), not a real upstream.
...
///
/// Only rejects the current XIASS API Service client/gateway URL (same host/port
...
/// Look up real provider Base URL from a XIASS data directory store.
...
/// Look up real provider Base URL from XIASS's model-provider store.
...
fn sidecar_codex_key_config_value_with_metered_feature_patterns(
    ...
    // second hop (XIASS → OpenAI-compatible upstream such as Sub2API) can stay on
