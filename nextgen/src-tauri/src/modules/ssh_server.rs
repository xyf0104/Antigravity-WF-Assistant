...
pub async fn test_connection(server_id: &str) -> Result<String, String> {
    ...
    let output = run_ssh(
        ...
        &["printf", "xiass-tools-ssh-ok"],
        None,
    )
    .await?;
    if output.trim() == "xiass-tools-ssh-ok" {
        Ok(output)
    } else {
...
mod tests {
    ...
    async fn live_ssh_own_syncs_current_codex_account() {
        let live_sync = std::env::var("XIASS_LIVE_SSH_OWN_SYNC")
            .or_else(|_| std::env::var("COCKPIT_LIVE_SSH_OWN_SYNC"))
            .ok();
        if live_sync.as_deref() != Some("1") {
            eprintln!("set XIASS_LIVE_SSH_OWN_SYNC=1 to run the live own SSH sync test");
            return;
        }
