...
impl WebdavSyncClient {
    ...
    pub async fn upload_backup_bytes(
        ...
        if !is_backup_file_name(file_name) {
            return Err("WebDAV 只允许上传 XIASS Tools 备份文件".to_string());
        }
    ...
    pub async fn read_remote_backup_bytes(&self, file_name: &str) -> Result<Vec<u8>, String> {
        if !is_backup_file_name(file_name) {
            return Err("WebDAV 只允许读取 XIASS Tools 备份文件".to_string());
        }
        let path = format!("{}/{}", self.remote_dir, file_name);
        let response = self
            .client
    ...
    pub async fn delete_remote_backup(&self, file_name: &str) -> Result<(), String> {
        if !is_backup_file_name(file_name) {
            return Err("WebDAV 只允许删除 XIASS Tools 备份文件".to_string());
        }
        let path = format!("{}/{}", self.remote_dir, file_name);
...
pub fn is_backup_file_name(file_name: &str) -> bool {
    ...
    let matches_prefix = trimmed.starts_with("xiass_auto_backup_")
        || trimmed.starts_with("xiass_manual_backup_")
        // Read-only compatibility for backup names created by earlier releases.
        || trimmed.starts_with("cockpit_auto_backup_")
...
#[cfg(test)]
mod tests {
    use super::{is_backup_file_name, normalize_base_url, normalize_remote_dir};

    #[test]
    fn accepts_xiass_names_and_legacy_cockpit_names() {
        assert!(is_backup_file_name("xiass_auto_backup_full_20260831.json"));
        assert!(is_backup_file_name(
            "xiass_manual_backup_accounts_20260831.zip"
        ));
        assert!(is_backup_file_name(
            "cockpit_auto_backup_full_20260831.json"
        ));
        assert!(is_backup_file_name(
            "cockpit_manual_backup_accounts_20260831.zip"
        ));
        assert!(!is_backup_file_name("other_auto_backup_full_20260831.json"));
    }
    ...
    fn normalize_webdav_target_trims_valid_values() {
        ...
        assert_eq!(
            normalize_remote_dir(" /xiass-tools/ ").unwrap(),
            "xiass-tools"
        );
    }
}
