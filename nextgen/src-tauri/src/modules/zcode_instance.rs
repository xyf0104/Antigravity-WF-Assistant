...
pub fn get_instances_root_dir() -> Result<PathBuf, String> {
    Ok(dirs::home_dir()
        .ok_or_else(|| "无法获取用户主目录".to_string())?
        .join(".xiass_tools/instances/zcode"))
}
...
mod tests {
    ...
    fn process_collection_uses_helper_user_data_dir_after_main_rewrites_argv() {
        let output = r#"
37764     1 ZCode [333]
37767 37764 /Applications/ZCode.app/Contents/Frameworks/ZCode Helper.app/Contents/MacOS/ZCode Helper --type=gpu-process --user-data-dir=/Users/test/.xiass_tools/instances/zcode/managed/electron --shared-files
41440     1 ZCode
...
"#;
        assert_eq!(
            ...
            vec![
                (
                    37764,
                    Some("/Users/test/.xiass_tools/instances/zcode/managed/electron".to_string())
                ),
                (
                    41440,
    ...
    fn pid_resolution_distinguishes_default_and_managed_user_data_dirs() {
        ...
        let managed_root = Path::new("/Users/test/.xiass_tools/instances/zcode/managed");
        let entries = vec![
            (
                37764,
