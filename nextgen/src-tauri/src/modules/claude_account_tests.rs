...
async fn cloudflare_challenge_uses_hidden_electron_probe_result() {
    ...
    let resolved =
        resolve_desktop_web_profile_with_hidden_probe("claude-desktop-test", Ok(direct), || {
...
fn extracts_desktop_profile_snapshot_id_from_legacy_paths() {
    ...
    assert_eq!(
            desktop_profile_snapshot_id_from_path(Path::new(
                r"C:\Users\Lenovo\.xiass_tools\claude_desktop_profiles\claude_desktop_0b1d3d4df02c2376d62a623bb8c67332"
            ))
            .as_deref(),
            Some(snapshot_id)
        );
...
#[test]
fn embedded_claude_code_projection_is_bounded_redacted_and_injectable_only() {
    let mut api_key = test_desktop_account(
        "claude_api_projection",
        "XIASS Claude API",
        None,
        None,
        10,
    ...
    api_key.auth_mode = ClaudeAuthMode::ApiKey;
    api_key.api_key = Some("api-key-must-not-cross-the-bridge".to_string());
    api_key.api_base_url = Some("https://private.example.test/v1".to_string());
    api_key.api_model_catalog = Some(vec![
        "claude-valid-model".to_string(),
        "not a model".to_string(),
        "CLAUDE-VALID-MODEL".to_string(),
    ]);

    let mut incomplete_oauth = test_desktop_account(
        "claude_oauth_incomplete",
        "Incomplete OAuth",
        None,
        None,
        10,
    ...
    incomplete_oauth.auth_mode = ClaudeAuthMode::OAuth;
    incomplete_oauth.claude_credentials_raw = Some(serde_json::json!({
        "claudeAiOauth": { "accessToken": "must-not-project" }
    }));

    let desktop = test_desktop_account(
        "claude_desktop_excluded",
        "Desktop profile",
        None,
    ...
    let candidates =
        embedded_claude_code_candidates_from_accounts(&[desktop, incomplete_oauth, api_key]);
    assert_eq!(candidates.len(), 1);
    let candidate = &candidates[0];
    assert_eq!(candidate.id, "claude_api_projection");
    assert_eq!(candidate.label, "XIASS Claude API");
    assert_eq!(candidate.credential_mode, "API Key");
    assert_eq!(candidate.models.len(), 1);
    assert_eq!(candidate.models[0].id, "claude-valid-model");

    let encoded = serde_json::to_string(&candidates).expect("encode projection");
    for forbidden in [
        "api-key-must-not-cross-the-bridge",
        ...
        assert!(
            ...
            "embedded projection leaked {forbidden}: {encoded}"
        );
    }
}

#[test]
fn embedded_claude_code_commit_survives_current_account_marker_failure() {
    let inject_calls = std::cell::Cell::new(0);
    let marker_calls = std::cell::Cell::new(0);

    let result = finalize_embedded_claude_code_account_application(
        || {
            inject_calls.set(inject_calls.get() + 1);
            Ok(())
        },
        || {
            marker_calls.set(marker_calls.get() + 1);
            Err("simulated provider current-state write failure".to_string())
        },
    )
    .expect("a committed credential injection must remain successful");

    assert_eq!(inject_calls.get(), 1);
    assert_eq!(marker_calls.get(), 1);
    assert!(result.ok);
    assert!(result.message.contains("不影响 Claude Code 使用"));
}

#[test]
fn embedded_claude_code_rejection_does_not_mark_current_account() {
    let marker_calls = std::cell::Cell::new(0);

    let result = finalize_embedded_claude_code_account_application(
        || Err("simulated credential injection failure".to_string()),
        || {
            marker_calls.set(marker_calls.get() + 1);
            Ok(())
        },
...
#[test]
fn claude_account_usage_tracking_failure_is_best_effort_after_commit() {
    let account = test_desktop_account(
    ...
    let persisted_last_used = std::cell::Cell::new(0);

    let recorded = record_claude_account_usage_after_injection(&account, |updated| {
        assert_eq!(updated.id, account.id);
        assert!(updated.last_used >= account.last_used);
        persisted_last_used.set(updated.last_used);
        Err("simulated account index write failure".to_string())
    });

    assert!(!recorded);
    assert!(persisted_last_used.get() >= account.last_used);
    assert_eq!(account.last_used, 20);
}
