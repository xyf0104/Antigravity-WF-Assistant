...
async fn run_refresh_for_service(policy: ServiceRefreshPolicy) -> Result<(), String> {
    match policy.key {
        ...
        "codex" => super::codex_quota::refresh_all_quotas_for_background()
            .await
            .map(|_| ()),
...
fn render_markdown(meta: &ReportMeta, rows: &[ReportRow]) -> String {
    ...
    output.push_str("# XIASS Tools Usage Report\n\n");
...
fn render_html(meta: &ReportMeta, rows: &[ReportRow]) -> String {
    ...
    output.push_str("<title>XIASS Tools Usage Report</title>");
    output.push_str(
    ...
    output.push_str("<h1>XIASS Tools Usage Report</h1>");
