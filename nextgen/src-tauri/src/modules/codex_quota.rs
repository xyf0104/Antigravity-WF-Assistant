...
const COCKPIT_API_PLAN_TYPE: &str = "XIASS API";
const LEGACY_COCKPIT_API_PLAN_TYPE: &str = "Cockpit Api";
        ...
        || account
            ...
            .map(|value| {
                ...
                value.eq_ignore_ascii_case(COCKPIT_API_PLAN_TYPE)
                    || value.eq_ignore_ascii_case(LEGACY_COCKPIT_API_PLAN_TYPE)
...
fn is_cockpit_api_base_url(raw: Option<&str>) -> bool {
    ...
    [COCKPIT_API_BASE_URL, LEGACY_COCKPIT_API_BASE_URL]
        .iter()
        .filter_map(|value| normalize_api_base_url_for_match(Some(value)))
...
fn build_new_api_profile_url(account: &CodexAccount) -> Result<String, String> {
    let base_url = account
        ...
        .ok_or("XIASS API 账号缺少 Base URL")?;
    let mut parsed =
        reqwest::Url::parse(base_url).map_err(|err| format!("XIASS API Base URL 无效: {}", err))?;
    if !matches!(parsed.scheme(), "http" | "https") {
...
async fn fetch_new_api_quota(account: &CodexAccount) -> Result<FetchQuotaResult, String> {
    let api_key = account
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or("XIASS API 账号缺少 OPENAI_API_KEY")?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("请求 XIASS API 额度失败: {}", err))?;
    let status = response.status();
    let body = response
        .text()
        .await
        .map_err(|err| format!("读取 XIASS API 额度响应失败: {}", err))?;
    if !status.is_success() {
        return Err(format!("XIASS API 额度接口返回 HTTP {}", status.as_u16()));
    }

    let root: serde_json::Value = serde_json::from_str(&body)
        .map_err(|err| format!("解析 XIASS API 额度 JSON 失败: {}", err))?;
    if root.get("success").and_then(|item| item.as_bool()) == Some(false) {
        let message = root
            .get("message")
            .and_then(|item| item.as_str())
            .unwrap_or("XIASS API 额度接口返回失败");
        return Err(message.to_string());
    }
    let data = root.get("data").unwrap_or(&root);
    let usage = data.get("usage").ok_or("XIASS API 额度响应缺少 usage")?;
            ...
            let result = match fetch_new_api_quota(&account).await {
                ...
                Err(e) => {
                    ...
                    if let Err(save_err) = codex_account::save_account(&account) {
                        logger::log_warn(&format!("写入 XIASS API 配额错误失败: {}", save_err));
                    }
                    return Err(e);
                }
            };
...
mod tests {
    ...
    #[test]
    fn recognizes_xiass_api_and_legacy_base_urls() {
