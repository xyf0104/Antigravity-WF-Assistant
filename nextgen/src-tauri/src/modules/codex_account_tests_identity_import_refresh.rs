...
{"id_token":"id-2","access_token":"access-2"}"#;
    ...
    impl TestEnvGuard {
        fn new(prefix: &str) -> Self {
            ...
            let test_data_dir = home_dir.join(".xiass_tools");
            ...
            std::env::set_var("XIASS_TOOLS_DATA_DIR", &test_data_dir);
    ...
    fn token_refresh_file_lock_is_scoped_to_install_data_dir() {
        ...
        assert!(path.starts_with(env.home_dir.join(".xiass_tools/.cockpit-token-locks")));
    ...
    fn profile_mutation_lock_allows_one_writer_and_rejects_the_concurrent_writer() {
        ...
        assert!(second.contains("另一个 XIASS Tools 环境正在操作"));
        ...
        let content = r#"[
  {
    ...
    fn quota_refresh_ownership_errors_are_internal_only() {
        assert!(super::is_refresh_ownership_deferred_error(
            "官方 ChatGPT/Codex 客户端正在使用此账号；为避免重复轮换 refresh_token，XIASS Tools 已暂停该账号刷新。"
        ));
        assert!(super::is_refresh_ownership_deferred_error(
