...
async fn load_remote_config_raw(
    _force_refresh: bool,
) -> Result<(RemoteConfigPayload, i64), String> {
...
mod tests {
    ...
    #[test]
    fn empty_payload_has_no_remote_policy_or_hidden_platforms() {
        let state = build_state(empty_payload(), 0);
        assert!(state.version.is_empty());
        assert!(state.hidden_platform_ids.is_empty());
        assert!(state.applied_rules.is_empty());
        assert_eq!(state.update_prompt_mode, UPDATE_PROMPT_MODE_NORMAL);
    }
