...
async fn request_trae_json(
    ...
    let mut request = client
        ...
        .header("User-Agent", "Trae/1.0.0 xiass-tools")
...
fn format_exchange_token_failure(official_err: &str, legacy_err: &str) -> String {
    ...
    {
        format!(
            "{}。可能原因：官方客户端已刷新并轮换了 RefreshToken，或设备密钥/区域不一致导致互刷顶号。请在 Trae 重新登录后于 XIASS Tools「从本地导入」，并避免 Trae 运行中对同一账号做登录刷新。",
            combined
        )
    } else {
        combined
...
async fn refresh_account_async_once(account_id: &str) -> Result<TraeAccount, String> {
    ...
    if normalize_non_empty(account.refresh_token.as_deref()).is_none() {
        return Err("Trae refresh token 缺失，无法按官方流程刷新登录态。请在 Trae 登录后于 XIASS Tools「从本地导入」。".to_string());
    }
