...
fn stable_prompt_cache_key(api_key: &ResolvedLocalApiKey) -> String {
    stable_uuid_from_text(&format!("xiass-tools:codex:prompt-cache:{}", api_key.id))
}

fn stable_codex_installation_id(api_key: &ResolvedLocalApiKey) -> String {
    stable_uuid_from_text(&format!("xiass-tools:codex:installation:{}", api_key.id))
}

fn stable_codex_turn_id(api_key: &ResolvedLocalApiKey, session_id: &str) -> String {
    stable_uuid_from_text(&format!("xiass-tools:codex:turn:{}:{}", api_key.id, session_id))
}
