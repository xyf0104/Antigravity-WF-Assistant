...
fn command_trace_enabled() -> bool {
    if let Ok(value) =
        std::env::var("XIASS_COMMAND_TRACE").or_else(|_| std::env::var("COCKPIT_COMMAND_TRACE"))
    {
...
fn should_detach_child() -> bool {
    if let Ok(value) =
        std::env::var("XIASS_CHILD_LOGS").or_else(|_| std::env::var("COCKPIT_CHILD_LOGS"))
    {
    ...
    if let Ok(value) =
        std::env::var("XIASS_CHILD_DETACH").or_else(|_| std::env::var("COCKPIT_CHILD_DETACH"))
    {
        let lowered = value.trim().to_lowercase();
