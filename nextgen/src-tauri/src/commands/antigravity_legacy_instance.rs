...
fn is_profile_initialized(user_data_dir: &str) -> bool {
    modules::antigravity_legacy_instance::is_profile_initialized(Path::new(user_data_dir))
}

fn resolve_default_account_id(settings: &DefaultInstanceSettings) -> Option<String> {
    if settings.follow_local_account {
        resolve_local_account_id().or_else(|| modules::get_current_account_id().ok().flatten())
    } else {
...
) -> Result<modules::antigravity_legacy_instance::InstanceDefaults, String> {
    modules::antigravity_legacy_instance::get_instance_defaults()
}

#[tauri::command]
pub async fn antigravity_legacy_list_instances() -> Result<Vec<InstanceProfileView>, String> {
    let store = modules::antigravity_legacy_instance::load_instance_store()?;
    let default_settings = store.default_settings.clone();
    let process_entries = modules::process::collect_antigravity_legacy_process_entries();
    let mut result: Vec<InstanceProfileView> = store
        .instances
        .into_iter()
        .map(|instance| {
            let resolved_pid = modules::process::resolve_antigravity_legacy_pid_from_entries(
                instance.last_pid,
    ...
    let default_pid = modules::process::resolve_antigravity_legacy_pid_from_entries(
...
pub async fn antigravity_legacy_create_instance(
    ...
    let instance = modules::antigravity_legacy_instance::create_instance(
        modules::antigravity_legacy_instance::CreateInstanceParams {
            working_dir: None,
            name,
            user_data_dir,
...
pub async fn antigravity_legacy_update_instance(
    ...
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = modules::antigravity_legacy_instance::get_default_user_data_dir()?;
        let default_dir_str = default_dir.to_string_lossy().to_string();
        let updated = modules::antigravity_legacy_instance::update_default_settings(
            bind_account_id,
            extra_args,
    ...
    let instance = modules::antigravity_legacy_instance::update_instance(
        modules::antigravity_legacy_instance::UpdateInstanceParams {
            working_dir: None,
            instance_id,
            name,
            extra_args,
...
pub async fn antigravity_legacy_delete_instance(instance_id: String) -> Result<(), String> {
    ...
    modules::antigravity_legacy_instance::delete_instance(&instance_id)
}
...
pub async fn antigravity_legacy_start_instance(
    ...
    if instance_id == DEFAULT_INSTANCE_ID {
        ...
        let default_bind_account_id = resolve_default_account_id(&default_settings);
        if let Some(pid) =
            modules::process::resolve_antigravity_legacy_pid(default_settings.last_pid, None)
        {
        ...
        let pid = modules::process::start_antigravity_legacy_with_args("", &extra_args)?;
    ...
    if let Some(pid) = modules::process::resolve_antigravity_legacy_pid(
        instance.last_pid,
        Some(&instance.user_data_dir),
    ) {
        modules::process::close_pid(pid, 20)?;
    ...
    if let Some(ref account_id) = instance.bind_account_id {
        ...
        modules::antigravity_legacy_instance::inject_account_to_profile(&profile_dir, account_id)?;
    }

    let extra_args = modules::process::parse_extra_args(&instance.extra_args);
    let pid =
        modules::process::start_antigravity_legacy_with_args(&instance.user_data_dir, &extra_args)?;
    let updated =
        modules::antigravity_legacy_instance::update_instance_after_start(&instance.id, pid)?;
    let running = modules::process::is_pid_running(pid);
...
pub async fn antigravity_legacy_close_all_instances() -> Result<(), String> {
    ...
    target_dirs.push(default_dir_str.clone());
    ...
    modules::process::close_antigravity_legacy_instances(&target_dirs, &default_dir_str, 20)?;
    let _ = modules::antigravity_legacy_instance::clear_all_pids();
    Ok(())
}
...
pub async fn antigravity_legacy_open_instance_window(instance_id: String) -> Result<(), String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_settings = modules::antigravity_legacy_instance::load_default_settings()?;
        modules::process::focus_antigravity_legacy_instance(default_settings.last_pid, None)
    ...
    modules::process::focus_antigravity_legacy_instance(
        instance.last_pid,
        Some(&instance.user_data_dir),
    )
    .map_err(|err| {
