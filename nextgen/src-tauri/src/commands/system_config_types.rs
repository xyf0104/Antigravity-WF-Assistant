...
fn read_powershell_json_for_antigravity_exe(
    ...
    let output = command
        ...
        .env("XIASS_ANTIGRAVITY_EXE_PATH", exe_path.as_os_str())
        .output()
        .ok()?;
    if !output.status.success() {
...
$exe = [Environment]::GetEnvironmentVariable('XIASS_ANTIGRAVITY_EXE_PATH', 'Process')
...
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$OutputEncoding = [System.Text.Encoding]::UTF8
$p = [Environment]::GetEnvironmentVariable('XIASS_ANTIGRAVITY_EXE_PATH', 'Process')
