...
func main() {
	...
	manifestPath := flag.String("manifest", "", "XIASS sidecar manifest file")
	quotaReserveStatePath := flag.String("quota-reserve-state", "", "XIASS OAuth quota reserve state file")
	quotaPoolStatePath := flag.String("quota-pool-state", "", "XIASS account-pool quota state file")
	parentPID := flag.Int("parent-pid", 0, "XIASS Tools parent process id")
	flag.Parse()
