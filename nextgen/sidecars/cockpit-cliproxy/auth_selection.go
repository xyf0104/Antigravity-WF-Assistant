...
func authPoolUnavailableError(locale string, stats authPoolSelectionStats, detail string) *coreauth.Error {
	...
	var message string
	if chinese {
		message = fmt.Sprintf("账号池没有可用账号：候选 %d 个，不可用 %d 个，模型排除 %d 个，额度保留拦截 %d 个，生图策略拦截 %d 个。请前往 XIASS Tools 查看账号池诊断详情。",
		...
		message = fmt.Sprintf("No available account: candidates=%d, unavailable=%d, model_excluded=%d, quota_reserved=%d, image_policy_blocked=%d. Check account pool diagnostics in XIASS Tools.",
			stats.candidateAuths, stats.unavailableAuths, stats.modelExcludedAuths, stats.quotaReservedAuths, stats.imagePolicyBlockedAuths)
	}
