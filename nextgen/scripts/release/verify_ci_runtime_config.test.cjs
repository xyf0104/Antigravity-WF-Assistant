...
test('Windows clean-install policy keeps offline WebView2 and exercises both installers in CI', () => {
  ...
  assert.match(installerSmoke, /function Export-XiassMsiBinary/);
  assert.match(installerSmoke, /SELECT `Name`, `Data` FROM `Binary` WHERE `Name`/);
  assert.match(installerSmoke, /\.ReadStream\(2, \$chunkLength, 1\)/);
  assert.match(installerSmoke, /function Invoke-XiassOfflineWebView2Prerequisite/);
  assert.match(installerSmoke, /Invoke-XiassOfflineWebView2Prerequisite \$msi\[0\] \$root/);
  assert.match(installerSmoke, /\@\('\/silent', '\/install'\)/);
  assert.match(installerSmoke, /Wait-XiassWebView2Runtime/);
  assert.match(installerSmoke, /MSI Binary table export is not a PE executable payload/);
...
test('platform CI workflows launch built applications before uploading artifacts', () => {
  ...
  assert.match(windowsInstallerSmoke, /function Export-XiassMsiBinary/);
  assert.match(windowsInstallerSmoke, /function Get-XiassWebView2RuntimeVersions/);
  assert.match(windowsInstallerSmoke, /function Invoke-XiassOfflineWebView2Prerequisite/);
