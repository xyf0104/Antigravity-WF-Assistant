...
该安装包已包含完整 x64 主程序，运行时不需要 Python、Node.js 或外置补丁脚本。支持 Windows 10/11 x64。安装前会按 Microsoft 官方注册表规则检查 Evergreen WebView2 Runtime；缺失时只从 Microsoft 官方地址下载小型 Bootstrapper，验证 Microsoft 数字签名后静默安装并再次检查运行时版本。若网络、代理或企业策略阻止下载或安装，助手本体会停止安装并显示原因，不会在首次打开时留下白屏。安装包不会捆绑体积庞大的 Fixed Version Runtime。
...
安装路径会从常见目录、运行进程、卸载注册表和便携目录中发现。renderer 图片预览补丁仅修改已识别的 JavaScript 结构，并保留原始文件备份、失败回滚与“恢复原始文件”流程；未知或结构变化的 Antigravity 版本会被安全跳过，不应手动套用其他版本的 renderer 文件。
