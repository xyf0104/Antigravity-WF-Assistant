...
func TestSelectHighestStableReleaseUsesSemanticVersionAndPlatformAsset(t *testing.T) {
	releases := []githubRelease{
		...
		testRelease("1.5.3", "2026-08-06T00:00:00Z", "windows"),
	...
	if got := normalizeVersion(windowsRelease.TagName); got != "1.5.3" {
		t.Fatalf("Windows selected %s, want 1.5.3", got)
	}
}
...
func TestSelectInstallerRequiresCanonicalReleaseAsset(t *testing.T) {
	release := githubRelease{
		...
		Assets: []githubAsset{
			...
			{Name: "Antigravity-WF-Assistant-macOS-v1.5.2-Installer.pkg"},
			{Name: "Antigravity-WF-Assistant-Windows-x64-v1.5.2-Portable.exe"},
	...
	canonical := testInstallerAsset("1.5.2", "darwin")
...
func TestDownloadLatestInstallerUsesSameHighestSemanticRelease(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/":
			...
			newInstaller.BrowserDownloadURL = server.URL + "/v153.installer"
			oldInstaller.Size = 1
			...
			releases := []githubRelease{
				...
				},
				{
					...
					Assets: []githubAsset{
						newInstaller,
						{Name: "SHA256SUMS.txt", BrowserDownloadURL: server.URL + "/v153.sha", Size: 1},
					},
				},
			}
			_, _ = writer.Write(marshalReleaseList(t, releases))
