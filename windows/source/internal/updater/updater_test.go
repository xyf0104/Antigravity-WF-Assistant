...
const testReleaseJSON = `{
  "tag_name":"v1.4.7",
  "html_url":"https://example.test/releases/v1.4.7",
  "published_at":"2026-08-04T00:00:00Z",
  "assets":[
    {"name":"Antigravity-WF-Assistant-macOS-universal-v1.4.7-Installer.pkg","browser_download_url":"https://example.test/macos.pkg","size":123},
    {"name":"Antigravity-WF-Assistant-Windows-x64-v1.4.7-Setup.exe","browser_download_url":"https://example.test/windows.exe","size":456}
  ]
}`
...
func TestCheckWithCacheUsesETagAnd304(t *testing.T) {
	...
	if !first.Available || first.Cached || first.LatestVersion != "1.4.7" || first.CheckedAt == "" {
	...
	if !second.Available || !second.Cached || second.CacheReason != "fresh" || second.LatestVersion != "1.4.7" || second.CheckedAt == "" {
	...
	if !third.Available || third.Cached || third.LatestVersion != "1.4.7" || third.CheckedAt == "" {
...
func TestCheckWithCacheFallsBackAfterServerFailure(t *testing.T) {
	...
	if !info.Cached || info.CacheReason != "network" || !info.Available || info.LatestVersion != "1.4.7" {
...
func TestCheckWithCacheMarksTimeoutFallback(t *testing.T) {
	...
	if !info.Cached || info.CacheReason != "timeout" || info.LatestVersion != "1.4.7" {
