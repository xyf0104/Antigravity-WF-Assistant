...
func FetchQuota(ctx context.Context, config Config, quotaURL string) QuotaResult {
	...
	result := QuotaResult{Endpoint: quotaURL, StatusCode: response.StatusCode, Snapshot: snapshot}
...
func quotaSnapshotFromResponse(response *http.Response, body []byte) storage.QuotaSnapshot {
	snapshot := storage.QuotaSnapshot{Source: "用户配置的额度接口", UpdatedAt: time.Now().UTC().Format(time.RFC3339), StatusCode: response.StatusCode}
	if response != nil {
		snapshot.RequestsRemaining = quotaHeader(response.Header, "X-RateLimit-Remaining-Requests", "Anthropic-Ratelimit-Requests-Remaining")
		snapshot.TokensRemaining = quotaHeader(response.Header, "X-RateLimit-Remaining-Tokens", "Anthropic-Ratelimit-Tokens-Remaining")
		snapshot.RequestsReset = quotaHeader(response.Header, "X-RateLimit-Reset-Requests", "Anthropic-Ratelimit-Requests-Reset")
		snapshot.TokensReset = quotaHeader(response.Header, "X-RateLimit-Reset-Tokens", "Anthropic-Ratelimit-Tokens-Reset")
	...
	snapshot.Available = snapshot.RequestsRemaining != "" || snapshot.TokensRemaining != "" || snapshot.RequestsReset != "" || snapshot.TokensReset != "" || snapshot.RetryAfter != ""
	return snapshot
}
