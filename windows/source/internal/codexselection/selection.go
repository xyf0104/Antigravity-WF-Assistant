...
func (service *Service) WithCredential(sessionID string, consume bool, use func(Credential) error) (State, error) {
	...
	credential := Credential{BaseURL: entry.baseURL, KeyName: entry.keyName, APIKey: append([]byte(nil), entry.apiKey...)}
...
func (entry *session) publicState() State {
	...
	return State{SessionID: entry.id, Status: entry.status, Message: entry.message, ExpiresAt: entry.expires.UTC().Format(time.RFC3339), BaseURL: entry.baseURL, KeyName: entry.keyName}
}
