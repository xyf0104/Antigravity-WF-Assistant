...
func injectCustomModels(parsed map[string]any, models []storage.CustomModel) modelInjectionSummary {
	...
	for _, model := range models {
		...
		// The picker entry for an OpenAI Chat model advertises image output so
		// Antigravity can invoke WF's gpt-image-2 bridge from SOL/Luna/Terra.
		// imageGenerationModelIds is different: it is an execution-model index,
		// and must contain only real image models or the IDE may execute the chat
		// model itself as an image backend.
