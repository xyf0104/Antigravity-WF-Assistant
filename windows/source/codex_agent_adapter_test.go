...
func TestCodexAgentSnapshotStatusIsConservative(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "valid xiass config", snapshot: codexconfig.ConfigSnapshot{Location: codexconfig.ConfigLocation{CodexHome: "/home/test/.codex", ConfigPath: "/home/test/.codex/config.toml", Exists: true}, Valid: true, ModelProvider: codexconfig.DefaultProviderID, ManagedProviderVerified: true}, homeExists: true, wantState: agent.StateReady, wantConfiguration: true},
		{name: "parseable but incomplete xiass provider is degraded", snapshot: codexconfig.ConfigSnapshot{Location: codexconfig.ConfigLocation{CodexHome: "/home/test/.codex", ConfigPath: "/home/test/.codex/config.toml", Exists: true}, Valid: true, ModelProvider: codexconfig.DefaultProviderID, ManagedProviderIssue: codexconfig.ManagedProviderIssueBearer}, homeExists: true, wantState: agent.StateDegraded, wantConfiguration: true},
...
func TestCodexDesktopStatusIsSeparateFromLocalConfig(t *testing.T) {
	metadata := newCodexAgentAdapter().Metadata()
	base := codexAgentSnapshotStatus(metadata, codexconfig.ConfigSnapshot{Location: codexconfig.ConfigLocation{CodexHome: "/home/test/.codex"}, Valid: true}, false, nil)
	decorated := decorateCodexDesktopStatus(base, codexdesktop.Status{State: codexdesktop.StateInstalled, Installation: codexdesktop.Installation{Present: true, Version: "1.8.0", ExecutableVerified: true}})
...
func TestCodexDesktopDegradedStateDoesNotClaimHistoryWritesAreSafe(t *testing.T) {
	metadata := newCodexAgentAdapter().Metadata()
	base := codexAgentSnapshotStatus(metadata, codexconfig.ConfigSnapshot{Location: codexconfig.ConfigLocation{CodexHome: "/home/test/.codex", Exists: true}, Valid: true}, true, nil)
