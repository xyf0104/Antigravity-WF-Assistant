...
- 修复了“代理刚启动就退出/重启”时极少数情况下旧监听器尚未释放的问题。现在 Windows 与 macOS 都会显式释放助手自己创建的监听器，随后可以立即安全重启；端口迁移的暂存与提交规则不变。
...
- 已验证更新安装程序启动后，助手会立即退出并释放程序文件与本地代理，不再等待固定延迟，降低 Windows 安装器替换仍在运行的主程序时失败的概率。
