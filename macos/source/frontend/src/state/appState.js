...
export const state = reactive({
  ...
  patchBusy: false,
  patchLog: "",
  // Connection work is serialized by the native App.  Keep the current
  // operation visible in the renderer so an in-flight safe patch never looks
  // like a stalled UI or invites a second click.
...
// Antigravity 2.0 has a separate safe connection route.  It deliberately uses
// the same renderer-wide busy state as the other two operations; the native
// App owns the final mutex so concurrent programmatic calls are safe too.
...
// Patch progress is emitted by the native serialized patch workflow.  Binding
// once prevents duplicate callbacks when the dashboard is revisited.
