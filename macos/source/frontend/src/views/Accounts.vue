...
async function saveAccount() {
  ...
  try {
		...
		if (!account.apiKey && !account.id && account.type !== "oauth") throw new Error("请填写 API Key、访问令牌，或使用 OAuth 授权/JSON 导入账户");
  } catch (error) {
...
function quotaSummary(account) {
	...
	const values = [];
	if (quota.requestsRemaining) values.push(`请求余量 ${quota.requestsRemaining}`);
	if (quota.tokensRemaining) values.push(`Token 余量 ${quota.tokensRemaining}`);
