...
const testReleaseJSON = `[{
  "tag_name":"v1.4.21",
  "html_url":"https://example.test/releases/v1.4.21",
  "published_at":"2026-08-04T00:00:00Z",
  "assets":[
    {"name":"Antigravity-WF-Assistant-macOS-universal-v1.4.21-Installer.pkg","browser_download_url":"https://example.test/macos.pkg","size":123},
    {"name":"Antigravity-WF-Assistant-Windows-x64-v1.4.21-Setup.exe","browser_download_url":"https://example.test/windows.exe","size":456}
  ]
}]`
...
func TestSelectHighestStableReleaseUsesSemanticVersionAndPlatformAsset(t *testing.T) {
	releases := []githubRelease{
		...
		testRelease("1.4.21", "2026-08-01T00:00:00Z", "darwin", "windows"),
		// The highest tag is not an update for macOS because its release lacks
		// the matching installer; it must not hide v1.4.21.
		testRelease("1.4.22", "2026-08-06T00:00:00Z", "windows"),
	...
	if got := normalizeVersion(macRelease.TagName); got != "1.4.21" {
		t.Fatalf("macOS selected %s, want 1.4.21", got)
	}
	...
	if got := normalizeVersion(windowsRelease.TagName); got != "1.4.22" {
		t.Fatalf("Windows selected %s, want 1.4.22", got)
	}
}
...
func TestCheckWithCacheUsesHighestSemanticVersionRatherThanPublicationTime(t *testing.T) {
	...
	if info.LatestVersion != "1.4.21" || !info.Available {
		t.Fatalf("check info = %#v, want available v1.4.21", info)
	}
}
...
func TestCheckWithCacheMigratesLegacySingleReleaseCache(t *testing.T) {
	...
	if requests != 1 || info.LatestVersion != "1.4.21" || !info.Available {
...
func TestDownloadLatestInstallerUsesSameHighestSemanticRelease(t *testing.T) {
		...
		case "/":
			...
			releases := []githubRelease{
				...
				},
				{
					...
					Assets: []githubAsset{
						newInstaller,
						{Name: "SHA256SUMS.txt", BrowserDownloadURL: server.URL + "/v121.sha", Size: 1},
					},
				},
			}
			_, _ = writer.Write(marshalReleaseList(t, releases))
	...
	if info.LatestVersion != "1.4.21" || info.AssetName != newInstaller.Name {
...
func TestCheckWithCacheUsesETagAnd304(t *testing.T) {
	...
	if !first.Available || first.Cached || first.LatestVersion != "1.4.21" || first.CheckedAt == "" {
	...
	if !second.Available || !second.Cached || second.CacheReason != "fresh" || second.LatestVersion != "1.4.21" || second.CheckedAt == "" {
	...
	if !third.Available || third.Cached || third.LatestVersion != "1.4.21" || third.CheckedAt == "" {
...
func TestCheckWithCacheFallsBackAfterServerFailure(t *testing.T) {
	...
	if !info.Cached || info.CacheReason != "network" || !info.Available || info.LatestVersion != "1.4.21" {
...
func TestCheckWithCacheMarksTimeoutFallback(t *testing.T) {
	...
	if !info.Cached || info.CacheReason != "timeout" || info.LatestVersion != "1.4.21" {
